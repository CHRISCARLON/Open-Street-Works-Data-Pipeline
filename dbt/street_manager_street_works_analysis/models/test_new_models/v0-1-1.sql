    ...
    base_permit_data AS (
        ...
        FROM
            (
                ...
                FROM
                    {{ source('street_manager', 'in_progress_works_list_england_latest') }} permit_data
                    LEFT JOIN {{ source('street_manager', 'uprn_usrn_counts_latest') }} uprn_usrn ON permit_data.usrn = uprn_usrn.usrn
                UNION ALL
                SELECT
                    permit_data.usrn,
                    permit_data.street_name,
                    ...
                    COALESCE(uprn_usrn.uprn_count, 0) as uprn_count
                FROM
                    {{ source('street_manager', 'completed_works_list_england_latest') }} permit_data
                    LEFT JOIN {{ source('street_manager', 'uprn_usrn_counts_latest') }} uprn_usrn ON permit_data.usrn = uprn_usrn.usrn
            ) AS combined_works
    ),
    raw_impact_level AS (
