...
def rename_columns(column_names: list[str]) -> list[str]:
    ...
    """
    return [col.replace("object_data.", "") if "object_data." in col else col for col in column_names]

def insert_table_to_motherduck(table: pa.Table, conn, schema: str, table_name: str) -> None:
    """
...
def batch_processor(zipped_chunks: Iterator, 
                    batch_size: int, 
                    conn, 
                    schema_name: str, 
                    table_name: str) -> None:
...
def process_data(url: str, batch_size: int, conn, schema_name: str, table_name: str) -> None:
    """
