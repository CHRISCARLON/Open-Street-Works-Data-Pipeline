...
def load_data_to_db(collected_rows, conn, schema, table_name):
    ...
    try:
        ...
        conn.execute(
            f"CREATE TABLE IF NOT EXISTS {full_table_name} (table_id VARCHAR, column_values JSON)"
        )
        ...
        for table_id, rows in collected_rows.items():
            ...
            if values:
                conn.execute(
                    f"INSERT INTO {full_table_name} (table_id, column_values) VALUES {', '.join(values)}"
                )
